                                 ...
                                 char *dst, unsigned int dstLen)
{
    ...
    for(i=0;i<numBuffers;i++){
        ...
        if (CPA_STATUS_SUCCESS == status)
        {
            ...
            pOpData->cryptoStartSrcOffsetInBytes = i*MAX_HW_BUFSZ/8;
            pOpData->messageLenToCipherInBytes = MAX_HW_BUFSZ/8;
            //</snippet>
        }
...
unlock:
    ...
    sessionSetupData.sessionPriority = CPA_CY_PRIORITY_NORMAL; // or _HIGH
    sessionSetupData.symOperation = CPA_CY_SYM_OP_CIPHER; // or _NONE or _HASH or _ALGORITHM_CHAINING
    sessionSetupData.cipherSetupData.cipherAlgorithm = CPA_CY_SYM_CIPHER_AES_ECB; // many other algorithms in doc P47
